...
parser.add_argument("--task", type=str, default="Cabinet-anubis-v0", help="Name of the task.")
...
from isaaclab.devices import Se3Keyboard_BMM  # For reset
...
def main():
    ...
    current_goal = None
    # Predefined goals (x, y)
    predefined_goals = [
        torch.tensor([0.3, 0.3], device=env.device),
        torch.tensor([-0.4, -0.5], device=env.device),
        torch.tensor([0.0, 0.0], device=env.device)
    ]
        ...
        while simulation_app.is_running():
            ...
            if running_recording_instance:
                if current_goal is None:
                    current_goal_index += 1
                    if current_goal_index >= len(predefined_goals):
                        print("All goals reached.")
                        break  # or loop: current_goal_index = 0
                    current_goal = predefined_goals[current_goal_index]
                    print(f"Switching to new goal: {current_goal.tolist()}")
                ...
                delta_pos = current_goal - base_pos[0,:2]
                ...
                if distance > 0.05:
                    if torch.abs(delta_yaw) > yaw_threshold:
                        delta_pose_base[:, 2] = 5 * torch.sign(delta_yaw)  # Rotate
                    else:
                        delta_pose_base[:, 0] = - 0.3  # Move forward  
                elif torch.abs(delta_yaw) > yaw_threshold:
                    delta_pose_base[:, 2] = 1 * torch.sign(delta_yaw)  # Rotate
                else:
                    print("Goal reached!")
                    current_goal = None  # Clear goal to ask for next
