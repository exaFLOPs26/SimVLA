...
parser.add_argument("--task", type=str, default="Isaac-Kitchen-v926-10", help="Name of the task.")
		...
		while simulation_app.is_running():
			...
			if torch.any(arm_r_mask):
				...
				if needs_plan_indices: 
					...
					# TODO arm_r_goals is (num_env, goal) and if the goal is a list of possible gool pose, we need to randomly pick one
					...
					# 3. Place
					if arm_r_goals[place_mask].numel() > 0 :
						arm_r_goals[place_mask][3:] = env.scene["robot"].data.body_quat_w[place_mask, 78]
...
#			start = time.time()
#			for sensor in sensors:
#				seg_tensor = env.scene.sensors.get(sensor).data.output["instance_id_segmentation_fast"][0]
#
#				# keep seg_tensor on GPU during simulation
#				# only clone ~F~R CPU when saving
...
#
#				# convert tuple keys to strings (e.g. "(25, 42, 255, 255)")
#				seg_labels_str = {str(k): v for k, v in seg_labels.items()}
#
#				# save label JSON
#				with open(
...
#					"w"
#				) as f:
#					json.dump(seg_labels_str, f, indent=2)
#				# === RGB ===
#				rgb_tensor = env.scene.sensors.get(sensor).data.output["rgb"]
#
#				if rgb_tensor.ndim == 4 and rgb_tensor.shape[0] == 1:
#					rgb_tensor = rgb_tensor[0]	 # [H, W, 4] or [H, W, 3]
#
#				rgb_to_save = rgb_tensor.to(torch.uint8).contiguous().clone().cpu().numpy()
#				imageio.imwrite(
#					os.path.join(output_root, f"rgb_{sensor}", f"rgb_{sensor}_{timestep:06d}.png"),
#					rgb_to_save
#				)
#			timestep += 1
