...
parser.add_argument(
	"--dataset_file", type=str, default="./datasets/anubis/kitchen_21.hdf5", help="File path to export recorded demos."
)
parser.add_argument("--step_hz", type=int, default=30, help="Environment stepping rate in Hz.")
parser.add_argument(
	"--num_demos", type=int, default=25, help="Number of demonstrations to record. Set to 0 for infinite."
)
parser.add_argument(
	"--num_success_steps",
	type=int,
...
# TODO: Check if this recorder matches to Lerobot format
	...
	success_step_count = torch.zeros(env.num_envs, device=env.device, dtype=torch.long)
	...
	goals_env0 = [
		...
		("A_r", torch.tensor([1.05875, -0.60828, 0.9534,-0.0039, -0.7134, 0.0066, -0.7008], device=device)),
		("G_r", torch.tensor( True, device=device)),
		("N", torch.tensor([0.32542, -0.68647, 0], device=device)),
	...
	goals_env2 = [
		 ...
		 ("A_r", torch.tensor([1.05875, -0.60828, 0.9534,-0.0039, -0.7134, 0.0066, -0.7008], device=device)),
		 ("G_r", torch.tensor( True, device=device)),
		 ("N", torch.tensor([0.32542, -0.63647, 0], device=device)),
		 ("G_r", torch.tensor( False, device=device)),
	 ]	
	goals_env3 = [
		 ("N", torch.tensor([0.67542, -0.63647, 0], device=device)),
	...
	all_goals = [goals_env0, goals_env1, goals_env2, goals_env3, goals_env4, goals_env5, goals_env6, goals_env7, goals_env8, goals_env9, goals_env10, goals_env11, goals_env12, goals_env13, goals_env14, goals_env15, goals_env16, goals_env17, goals_env18, goals_env19, goals_env20, goals_env21, goals_env22, goals_env23, goals_env24]
	...
	if args_cli.robot == "anubis":
		...
		world_cfg = usd_helper.get_obstacles_from_stage(
			only_paths=["collisions", "env_0"], reference_prim_path="/World/envs/env_0"
		)
		print(world_cfg)
