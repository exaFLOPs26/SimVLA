...
# TODO: Check if this recorder matches to Lerobot format
	...
	goals_env0 = [
		...
		("N", torch.tensor([0.42542, -0.68647, 0], device=device)),
	...
	goals_env2 = [
		 ("N", torch.tensor([0.67542, -0.63647, 0], device=device)),
		 ("A_r", torch.tensor([1.04875, -0.60828, 0.9534,-0.0039, -0.7134, 0.0066, -0.7008], device=device)),
		 ("G_r", torch.tensor( True, device=device)),
		 ("N", torch.tensor([0.42542, -0.63647, 0], device=device)),
		 ("G_r", torch.tensor( False, device=device)),
	 ]	
	goals_env3 = [
		 ("N", torch.tensor([0.67542, -0.63647, 0], device=device)),
...
#	 ]
	...
	all_goals = [goals_env0, goals_env1, goals_env2, goals_env3, goals_env4, goals_env5, goals_env6, goals_env7, goals_env8, goals_env9]
	...
	with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
		while simulation_app.is_running():
			...
			if torch.any(nav_mask):
				...
				# 3. Convert the robot's world position into its position relative to its own environment
			...
			gripper_command_R = new_gripper_commands_R
			...
			if torch.any(grip_l_mask):
                ...
                # The command should stay True for the current goal until the next goal starts.
                current_commands = (grip_r_goals[:, 0] > 0)
                new_gripper_commands_L[grip_r_env_indices] = current_commands
            ...
            gripper_command_R = new_gripper_commands_L
