...
parser.add_argument(
	"--dataset_file", type=str, default="./datasets/anubis/kitchen01.hdf5", help="File path to export recorded demos."
)
...
def torch_quat_to_rotvec(q, epsilon=1e-8):
	"""Converts a batch of quaternions to rotation vectors."""
	w, v = q[..., 0], q[..., 1:]
	angle = 2 * torch.atan2(torch.linalg.norm(v, dim=-1), w)
	axis = v / (torch.linalg.norm(v, dim=-1, keepdim=True) + epsilon)
...
def compute_eef_deltas_batched(motion_gen, plans, cmd_indices, joint_names, eef_link_name):
	...
	delta_rotvec = torch_quat_to_rotvec(delta_quat)
...
# TODO: Check if this recorder matches to Lerobot format
	...
	TASK_IDS = {"N": 0, "A_l": 1, "A_r": 2, "G_l": 3, "G_r": 4}
	N_ID, AL_ID, AR_ID, GL_ID, GR_ID = TASK_IDS["N"], TASK_IDS["A_l"], TASK_IDS["A_r"], TASK_IDS["G_l"], TASK_IDS["G_r"]

	# Goal states (World frame)
	goals_env0 = [
		("N", torch.tensor([0.67542, -0.68647, 0], device=device)),
		("A_r", torch.tensor([1.04875, -0.60828, 0.9534,-0.0039, -0.7134, 0.0066, -0.7008], device=device)),
		("G_r", torch.tensor( True, device=device)),
		("N", torch.tensor([0.52542, -0.68647, 0], device=device)),
		("G_r", torch.tensor( False, device=device)),
	]

	goals_env1 = [
		("N", torch.tensor([0.60542, -0.63647, 0], device=device)),
		("A_r", torch.tensor([1.04875, -0.60828, 0.9534,-0.0039, -0.7134, 0.0066, -0.7008], device=device)),
		("G_r", torch.tensor( True, device=device)),
		("N", torch.tensor([0.52542, -0.63647, 0], device=device)),
		("G_r", torch.tensor( False, device=device)),
	]

	# This is your main list containing the script for each environment
	all_goals = [goals_env0, goals_env1]
	...
	if args_cli.robot == "anubis":
		...
		world_cfg = usd_helper.get_obstacles_from_stage(
			only_paths=["collisions"], reference_prim_path="/world"
		)

		# --- Create a planner for the RIGHT arm ---
		...
		# --- Create a planner for the LEFT arm ---
	...
	with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
		while simulation_app.is_running():
			# Use inference_mode for the majority of the loop for performance.
			# Gradients are only enabled when calling the cuRobo planner.
			...
			gripper_command_L = torch.zeros(num_envs, dtype=torch.bool, device=device)
			gripper_command_R = torch.zeros(num_envs, dtype=torch.bool, device=device)

			finished_mask = torch.zeros(num_envs, dtype=torch.bool, device=device)
			env_indices = torch.arange(num_envs, device=device)
			current_task_ids = task_ids_tensor[env_indices, env_goal_indices]
			print(current_task_ids)
			# --- Create boolean masks for each task type ---
			...
			if torch.any(nav_mask):
				# 1. Get robot's absolute WORLD position and orientation
				root_pos_w = env.scene.articulations['robot'].data.root_link_pos_w[nav_mask, :2]
				w, x, y, z = env.scene.articulations['robot'].data.root_link_quat_w[nav_mask].unbind(-1)
				current_yaw = torch.atan2(2 * (w * z + x * y), 1 - 2 * (y * y + z * z))

				# 2. Get the origins for ONLY the navigating environments
				nav_env_origins = env.scene.env_origins[nav_mask]
				...
				# 4. Get the environment-relative goals
				nav_env_indices = env_indices[nav_mask]
				nav_goal_step_indices = env_goal_indices[nav_mask]
				nav_goals = payloads_tensor[nav_env_indices, nav_goal_step_indices]
				goal_pos_e, goal_yaw = nav_goals[:, :2], nav_goals[:, 2]

				# 5. Now the subtraction is correct because both positions are in the same environment frame
				delta_pos = goal_pos_e - base_pos_e
				distance = torch.linalg.norm(delta_pos, dim=1)
				# 6. The rest of the logic remains exactly the same
				...
				if torch.any(moving_mask):
					...
					temp_nav_mask = nav_mask.clone()
					temp_nav_mask[temp_nav_mask.clone()] = moving_mask
					delta_pose_base[temp_nav_mask] = nav_actions
				...
				if torch.any(rotating_mask):
					...
					angular_speed = torch.sign(delta_yaw_g) * 5.0
					
					rot_actions = torch.zeros(torch.sum(rotating_mask), 3, device=device)
					rot_actions[:, 2] = angular_speed

					temp_nav_mask = nav_mask.clone()
					temp_nav_mask[temp_nav_mask.clone()] = rotating_mask
					delta_pose_base[temp_nav_mask] = rot_actions

					is_aligned = torch.abs(delta_yaw_g) < goal_reached_yaw
					if torch.any(is_aligned):
						delta_pose_base[temp_nav_mask][is_aligned] = 0.0
						finished_rotating_mask = temp_nav_mask.clone()
						finished_rotating_mask[temp_nav_mask.clone()] = is_aligned
						finished_mask[finished_rotating_mask] = True

			# =================== Right Arm Motion ("A_r") ===================
			if torch.any(arm_r_mask):
				arm_r_indices = torch.where(arm_r_mask)[0]
				...
				if needs_plan_indices:
					...
					with torch.enable_grad():
						for i, env_idx in enumerate(needs_plan_indices):
							...
							# 2. FIX: Create the JointState with velocity data as in the original script
							cu_js = JointState(
							...
							# The rest of the planning logic continues as before
			...
			if torch.any(arm_l_mask):
				arm_l_indices = torch.where(arm_l_mask)[0]
				...
				if needs_plan_indices:
					...
					with torch.enable_grad(): # Enable gradients for cuRobo planning
						for i, env_idx in enumerate(needs_plan_indices):
			...
			if torch.any(grip_r_mask):
				grip_r_env_indices = env_indices[grip_r_mask]
				grip_r_goal_step_indices = env_goal_indices[grip_r_mask]

				# Select the correct payloads (containing 1.0 or -1.0) from the 3D tensor
				grip_r_goals = payloads_tensor[grip_r_env_indices, grip_r_goal_step_indices]
	
				# Set gripper command: True if payload is > 0, False otherwise	
				gripper_command_R[grip_r_mask] = (grip_r_goals[:, 0] > 0)
				r_gripper_pos = env.scene._articulations['robot'].data.body_pos_w[grip_r_mask][:, [79, 80]]
				r_dist_curr = torch.linalg.norm(r_gripper_pos[:, 0] - r_gripper_pos[:, 1], dim=1)
				is_stopped = (env_r_gripper_dist_prev[grip_r_mask] - r_dist_curr).abs() < 0.001
				temp_grip_r_mask = grip_r_mask.clone()
				temp_grip_r_mask[temp_grip_r_mask.clone()] = is_stopped
				finished_mask[temp_grip_r_mask] = True
				env_r_gripper_dist_prev[grip_r_mask] = r_dist_curr
			
			if torch.any(grip_l_mask):
				grip_l_env_indices = env_indices[grip_l_mask]
				grip_l_goal_step_indices = env_goal_indices[grip_l_mask]
				grip_l_goals = payloads_tensor[grip_l_env_indices, grip_l_goal_step_indices]
				gripper_command_L[grip_l_mask] = (grip_l_goals[:, 0] > 0)
				l_gripper_pos = env.scene._articulations['robot'].data.body_pos_w[grip_l_mask][:, [82, 83]]
				l_dist_curr = torch.linalg.norm(l_gripper_pos[:, 0] - l_gripper_pos[:, 1], dim=1)
				# Using the original completion condition for left gripper
				is_closed_enough = l_dist_curr < 0.005
				temp_grip_l_mask = grip_l_mask.clone()
				temp_grip_l_mask[temp_grip_l_mask.clone()] = is_closed_enough
				finished_mask[temp_grip_l_mask] = True

			# =================== Goal State Progression ===================
			if torch.any(finished_mask):
				finished_indices = torch.where(finished_mask)[0].tolist()
				for i in finished_indices: env_cmd_plans[i] = None
				env_cmd_indices[finished_mask] = 0
				env_r_gripper_dist_prev[finished_mask] = 10.0
				env_goal_indices[finished_mask] += 1
				
				if torch.all(env_goal_indices >= len(goals)):
				...
				if bool(success_term.func(env, **success_term.params)[0]):
					...
					if success_step_count >= args_cli.num_success_steps:
						...
						env.recorder_manager.set_success_to_episodes(
							[0], torch.tensor([[True]], dtype=torch.bool, device=device)
						)
