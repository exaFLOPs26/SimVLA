...
parser.add_argument(
	"--dataset_file", type=str, default="./datasets/anubis/kitchen1.hdf5", help="File path to export recorded demos."
)
...
def world2base(env, ee_pos_w, ee_quat_w) -> tuple[torch.Tensor, torch.Tensor]:
	...
	# TODO body_offset is in actioncfg , first ignore
#	if robot_data.cfg.body_offset is not None:
#		ee_pose_b, ee_quat_b = math_utils.combine_frame_transforms(
#			ee_pose_b, ee_quat_b, robot_data._offset_pos, robot_data._offset_rot
#		)
...
#goals.append(("A_r", torch.tensor([0.1, -0.2, 1.0, 0.6396, -0.5940, 0.3203, -0.3682], device=env.device)))
	...
	goals.append(("A_r", torch.tensor([1.02375, -0.60828, 0.9534, 0.4601, -0.5237,  0.5236, -0.4898], device=env.device)))
	goals.append(("G_r", torch.tensor( True, device=env.device)))  # Gripper close
	goals.append(("N", torch.tensor([0.57542, -0.68647, 0], device=env.device)))
...
#break	# or loop: base_goal_index = 0
				...
				if current_goal[0] == "N":		
					...
					if distance > 0.04:
						if torch.abs(delta_yaw) > yaw_threshold:
							...
							if distance > 0.1:
								delta_pose_base[:, 0] = -10.0
							else:
								delta_pose_base[:, 0] = -0.5  # Move forward
					elif torch.abs(delta_yaw_g) > yaw_threshold:
							delta_pose_base[:, 2] = 5.0 * torch.sign(delta_yaw_g)  # Rotate to match goal yaw
					else:	
						print("Goal reached!")
				...
				# Arm motion planning
					...
					if cmd_plan is None:
						...
						delta_r = r_tp1 * r_t.inv()
						delta_rotvec = torch.tensor(delta_r.as_rotvec(), dtype=torch.float32)
						pose_R = torch.cat((delta_pos, delta_rotvec))	
						cmd_idx += 1
				...
				# convert to torch
				pose_L = pose_L.clone().detach().to(env.device).repeat(env.num_envs, 1)
				pose_R = pose_R.clone().detach().to(env.device).repeat(env.num_envs, 1)
				delta_pose_base = delta_pose_base.clone().detach().to(env.device).repeat(env.num_envs, 1)
