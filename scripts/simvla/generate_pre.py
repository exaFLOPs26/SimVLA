"""
Pre-step: Whole-body motion planning with cuRobo
"""
...
from isaaclab.envs.mdp.recorders.recorders_cfg import PreActionStateRecorderManagerCfg
...
def main():
	...
	# Parse cartesian product of goal states
	...
	with open(file_path, 'r') as file:
		...
		for goal in goals[0]:
			new_all_goals = []
			if (isinstance(goal[1], list) and not any(isinstance(x, list) for x in goal[1])) or type(goal[1]) == bool:
				if not all_goals:
					all_goals.append([goal])
				else:
					for goal_in_all in all_goals:
						goal_in_all.append(goal)
						new_all_goals.append(goal_in_all.copy())
						goal_in_all.pop()
					all_goals = new_all_goals

			elif isinstance(goal[1], list) and any(isinstance(x, list) for x in goal[1]):
				for goal_in_all in all_goals:
					for subgoal in goal[1]:
						goal_in_all.append([goal[0], subgoal])
						new_all_goals.append(goal_in_all.copy())
						goal_in_all.pop()
				all_goals = new_all_goals
	num_envs = len(all_goals)
	device = args_cli.device
	
	for env in all_goals:
		for goal in env:
			goal[1] = torch.tensor(goal[1], dtype=torch.float32, device=device)	
	# Parse configuration
...
# TODO: Check if this recorder matches to Lerobot format
	env_cfg.recorders: PreActionStateRecorderManagerCfg = PreActionStateRecorderManagerCfg()
			...
			if torch.any(arm_r_mask):
				...
				if needs_plan_indices:
					...
					if torch.any(not_reset_and_not_place):
						reset_r[needs_plan_indices[grasp_env], :3] = env.scene["robot"].data.body_pos_w[needs_plan_indices[grasp_env], 78]
						reset_r[needs_plan_indices[grasp_env], 3:] = env.scene["robot"].data.body_quat_w[needs_plan_indices[grasp_env], 78]
					# 2. Reset
					if torch.any(reset_mask):
						arm_r_goals[reset_env] = reset_r[needs_plan_indices[reset_env]]
						arm_r_goals[reset_env, :3] -= env.scene.env_origins[needs_plan_indices[reset_env], :3]

					# 3. Place
					if torch.any(place_mask):
						arm_r_goals[place_env, 3:] = env.scene["robot"].data.body_quat_w[needs_plan_indices[place_env], 78]
