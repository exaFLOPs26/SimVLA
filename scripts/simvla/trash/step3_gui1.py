# Step 3: Read USD and make config file and goal generator (GUI Version - Final & Complete)
...
def make_walls_from_bounds(world_bounds, wall_material="wall_material", height=3.0, thickness=0.1):
	...
	args = {"width": x_max - x_min, "depth": y_max - y_min, "height": height, "thickness": thickness, "x_min": x_min, "y_min": y_min, "x_max": x_max, "y_max": y_max, "x_mid": (x_min + x_max) / 2, "y_mid": (y_min + y_max) / 2, "wall_material": wall_material}
	templates = [
		("wall_01", "{x_mid:.3f}, {y_min:.3f}, {height:.3f}", "(0.70711, 0.70711, 0.0, 0.0)", "{width:.3f}, {height}, {thickness}"),
		("wall_02", "{x_max:.3f}, {y_mid:.3f}, {height:.3f}", "(0.5, 0.5, 0.5, 0.5)", "{depth:.3f}, {height}, {thickness}"),
		("wall_03", "{x_min:.3f}, {y_mid:.3f}, {height:.3f}", "(0.5, 0.5, 0.5, 0.5)", "{depth:.3f}, {height}, {thickness}"),
		("wall_04", "{x_mid:.3f}, {y_max:.3f}, {height:.3f}", "(0.70711, 0.70711, 0.0, 0.0)", "{width:.3f}, {height}, {thickness}")
	]
...
class GoalGeneratorApp:
	def __init__(self, root):
		...
		setup_frame = ttk.LabelFrame(root, text="1. Kitchen Setup"); setup_frame.pack(fill="x", padx=10, pady=5, ipadx=5, ipady=5)
		goal_frame = ttk.LabelFrame(root, text="2. Goal Definition"); goal_frame.pack(fill="both", expand=True, padx=10, pady=5)
		log_frame = ttk.LabelFrame(root, text="Status Log"); log_frame.pack(fill="x", padx=10, pady=5)
		...
		self.load_goals_btn = ttk.Button(goal_btn_frame, text="Load Goals from File...", command=self.load_goals_from_file, state="disabled"); self.load_goals_btn.pack(pady=2, fill="x")
		...
		self.log_text = tk.Text(log_frame, height=8, wrap="word", relief="sunken", borderwidth=1); self.log_text.pack(fill="both", expand=True, padx=5, pady=5)
	...
	def process_kitchen(self):
		...
		if omni.usd.get_context().open_stage(usd_file_path):
			...
			self.log(f"ERROR: Failed to open {usd_file_path}"); messagebox.showerror("Error", f"Failed to open USD file:\n{usd_file_path}")
	...
	def _generate_env_config(self, kitchen_num_str, sub_num_str):
		try:
			...
			if k_data["kitchen_type"] == "single_wall":
				...
				furniture, exclude_names = [], ["floor", "wall", "ground"]
				for path in first_level_list:
					try:
						prim_name = path.split("/")[-1].lower()
						if any(name in prim_name for name in exclude_names): continue
	...
	def _write_env_config_file(self, kitchen_num_str, sub_num_str, first_level_list, world_bounds):
		...
		for path in first_level_list:
			try:
				...
				block = f"""{name} = ArticulationCfg(\n\t\t\t\t\tprim_path="{{ENV_REGEX_NS}}/Kitchen/{name}",\n\t\t\t\t\tspawn=None,\n\t\t\t\t\tinit_state=ArticulationCfg.InitialStateCfg(pos={pos}, rot={quat}, joint_pos={joint_pos}){actuator_block}\n\t\t\t\t)""" if joint_names else f"""{name} = RigidObjectCfg(\n\t\t\t\t\tprim_path="{{ENV_REGEX_NS}}/Kitchen/{name}",\n\t\t\t\t\tspawn=None,\n\t\t\t\t\tinit_state=RigidObjectCfg.InitialStateCfg(pos={pos}, rot={quat})\n\t\t\t\t)"""
	...
	def load_goals_from_file(self):
		...
		try:
			...
			self.log(f"Failed to load file: {e}"); messagebox.showerror("Load Error", f"An error occurred:\n{e}")
	...
	def save_goal_file(self):
		...
		for step in self.goal_steps:
			try:
				...
				self.log(f"Error processing step '{step['lang']}': {e}"); messagebox.showerror("Processing Error", f"Could not process step '{step['lang']}':\n{e}"); return
		...
		try:
			...
			self.log(f"ERROR saving file: {e}"); messagebox.showerror("Save Error", f"Could not save file:\n{e}")
	...
	def _process_goal_step(self, step):
		...
		if action in ["N", "N_s"]:
			...
			arm_bias = 0.15 if which_arm == "Left" else -0.15 if which_arm == "Right" else 0.0
			N_pos, safety, wheel_r = torch.zeros(3), 0.1, 0.23
			...
			else: # It's a small object - Using original raycast logic
			...
			if usage == "Move arm to reset":
				...
				if usage == "Move arm to grasp" and bodex_attr and bodex_attr.HasAuthoredValue():
					...
					grasp_file_path = pick_random_grasp_file(bodex_attr.Get())
					...
					rot_x = R.from_rotvec([np.pi/2, 0, 0]); rotated_xyz = rot_x.apply(xyz)
					new_rotations = rot_x * R.from_quat(quat_xyzw)
					
					angle_deg = int(self.kitchen_data["kitchen_sub_num"]) * 30
					rot_y = R.from_rotvec([0, np.radians(angle_deg), 0])
					...
					if len(avail_data) == 0: raise ValueError(f"No valid grasps found for approach direction '{self.N_dir}'")
					
					rand_data = random.choice(avail_data)
					pose_idx_mask = np.all(np.isclose(rotated_data, rand_data, atol=1e-6), axis=1)
					...
					rotated_point = rot_y.apply(rot_x.apply(mean_contact_point.numpy()))
					
					reference_frame = (torch.tensor(object_min_bound) + torch.tensor(object_max_bound)) / 2
					grasp_pos[0:3] = reference_frame + torch.tensor(rotated_point, dtype=torch.float32)
					grasp_pos[3:] = torch.tensor(rand_data[3:])
					processed_value = grasp_pos.tolist()
				else:
