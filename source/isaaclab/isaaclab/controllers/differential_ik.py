...
class DifferentialIKController:
	...
	def reset(self, env_ids: torch.Tensor = None):
		...
		if env_ids is None:
			...
			idx = torch.where(env_ids)[0]  
			if idx.numel() == 0:
				pass  # nothing selected
			else:
				self.ee_pos_des[idx] = torch.zeros(len(idx), 3, device=self._device)
				self.ee_quat_des[idx] = torch.zeros(len(idx), 4, device=self._device)
	def set_command(
		...
		if self.cfg.command_type == "position":
			...
			if self.cfg.use_relative_mode:
				...
				mask = torch.all(self.ee_pos_des == 0.0, dim=1)
				if torch.any(mask):
				    self.ee_pos_des[mask] = ee_pos[mask].clone()
				    self.ee_quat_des[mask] = ee_quat[mask].clone()
